...
void orientation() {
  ...
  currentX += delta_globalR * cos(currentTheta);
  currentY += delta_globalR * sin(currentTheta);
  currentTheta +=phi;
...
void driveForward() {
  ...
  float lineTouchingDistance = 215; // millimeters
  count_e0 = 0;
  count_e1 = 0;
...
void linefollowing(){
  ...
  else if(readLineSensor(2)<720 && avgDistance>2790){
    setMotorPower(0,0);
    orientation();    
    updateCoordinates();
...
void turnToOrigin() {
    ...
    float totalRotatedAngle = 0.0;
    
    while (totalRotatedAngle < degreesToRotate) {
        ...
        //updateCoordinates();
        orientation();
        totalRotatedAngle += degreesToRotate;
...
void backToOrigin(){
  ...
  distToOrigin = sqrt(currentX * currentX + currentY * currentY);
  // Calculate the number of encoder counts needed to travel the target distance
  float countsNeeded = (1385 / WHEEL_CIRCUMFERENCE) * CLICKS_PER_ROTATION * GEAR_RATIO;
  // because something is wrong in currentX and CurrentY and avg distamce is 1400 of between the points
  count_e0 = 0;
  count_e1 = 0;
  while (-count_e0 < countsNeeded || -count_e1 < countsNeeded) {
    setMotorPower(-22, -26.5);
  }
...
void loop(){
  ...
  switch (currentState) {
    ...
    case followingLine:
      ...
      if(readLineSensor(2)<720 && avgDistance>2790){
